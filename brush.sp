...
public OnPluginStart() {
	...
	HookConVarChange((hRandom = AutoExecConfig_CreateConVar("sm_brush_stripcsgocolors", "0",
	"Strip color codes of chat for CSGO?\n0 = No\n1 = Yes", FCVAR_NONE, true, 0.0, true, 1.0)), OnStripColorsChanged);
	StripColorsForCSGO = GetConVarBool(hRandom);
	SetAppend(appended);
...
public Action:Command_JoinTeam(client, const String:command[], argc) {
	...
	{
		#if _DEBUG
		Format(dmsg, sizeof(dmsg), "[Command_JoinTeam] Client [%i] is either invalid or not in game.");
		DebugMessage(dmsg);
		#endif
		return Plugin_Continue;
	}
	...
	{ // Plugin is switching the player, allow and stop checking anything beyond
		#if _DEBUG
		Format(dmsg, sizeof(dmsg), "[Command_JoinTeam] %N is allowed to join team %i because [SwitchingPlayer]", client, team);
		DebugMessage(dmsg);
		#endif
		return Plugin_Continue;
	}
	...
	// If human player is joining a team, let's check and reduce the bot count by 1 before they get processed
	if (team > CS_TEAM_SPECTATOR)
	{ // team is CS_TEAM_T or CS_TEAM_CT
		if (ManageBots && !IsFakeClient(client))
		{
			new quota = GetConVarInt(brush_botquota);
			if (quota >= 1)
			{ // There is 1 or more bots in game, let's remove a bot so the human player can take its slot.
				new botId = GetBotOnPlayerTeam(team);
				if (botId > 0)
				{ // There is a bot on the team the client just joined, let's kick that bot and allow the player to join the team and end it here
					KickClient(botId, "Kicked to make room for human player");
					
					#if _DEBUG
					Format(dmsg, sizeof(dmsg), "[Command_JoinTeam] Kicked %L from [%s] team because %L joined the team", botId, ((team == CS_TEAM_T) ? "Terrorists" : "CT"), client);
					DebugMessage(dmsg);
					#endif
				...
				{ // bot(s) must be on other team, let's just reduce the bot_quota by 1 and let the Timer_HandleTeamSwitch handle the switch
					quota--;
					bot_quota = quota;
					SetConVarInt(brush_botquota, bot_quota, true, true);
					
					#if _DEBUG
					Format(dmsg, sizeof(dmsg), "[Command_JoinTeam] No bots on [%s] team that %L joined.", ((team == CS_TEAM_T) ? "Terrorists" : "CT"), client);
					DebugMessage(dmsg);
					#endif
				}
			}
			else
			{ // Both teams are full of human players, let Timer_HandleTeamSwitch handle the switch to spectate and notification
				#if _DEBUG
				DebugMessage("[Command_JoinTeam] There are no bots currently in game.");
				#endif
			}
			
			CreateTimer(0.2, Timer_HandleTeamSwitch, GetClientSerial(client), TIMER_FLAG_NO_MAPCHANGE);
		}
	}
...
public Action:Timer_ClientDisconnected(Handle:timer) {
	...
	if ((Tcount+CTcount) < 8)
	{ // Teams have less than 8 players, let's add a bot since a player just left
		new quota = GetConVarInt(brush_botquota);
		if (quota < 8)
		{ // bot_quota is less than max players (8), increase bot_quota by 1
			quota++;
			bot_quota = quota;
...
public Action:Timer_HandleTeamSwitch(Handle:timer, any:serial) {
	...
	//new bool:TeamsFull; // Remember, extra player is already on the team at this point, so we just check if player count is greater than the allowed amount for each team
	//(((team == CS_TEAM_T && tCount > 5) && ctCount > 3) || ((team == CS_TEAM_CT && ctCount > 3) && tCount > 5)) ? (TeamsFull = true) : (TeamsFull = false);
	//(team == CS_TEAM_CT && ctCount > 3) ? (ctTeamFull = true) : (ctTeamFull = false);
	//(team == CS_TEAM_T && tCount > 5) ? (tTeamFull = true) : (tTeamFull = false);
		...
		((team == CS_TEAM_CT && ctCount > 3) && tCount >= 5))
	{
		...
		{ // Teams are already full, why is there a bot trying to join?
			#if _DEBUG
	...
	{ // There is room in one of the teams
		//new jointeam;
		/* if the player is on CT and CT count greater than 3, set jointeam to T, otherwise, set jointeam to CT */
		//(team == CS_TEAM_CT && ctCount > 3) ? (jointeam = CS_TEAM_T) : (jointeam = CS_TEAM_CT);
		
		//#if _DEBUG
		//Format(dmsg, sizeof(dmsg), "[HandleTeamSwitch] %L tried to join [%i] team, but it's full, switching to [%i]", client, team, jointeam);
		//DebugMessage(dmsg);
		//Format(dmsg, sizeof(dmsg), "[HandleTeamSwitch] %L tried to join [%s] team, but it's full, switching to [%s]", client, jointeam == CS_TEAM_CT ? "Terrorist" : "CT", jointeam == CS_TEAM_CT ? "CT" : "Terrorist");
		//DebugMessage(dmsg);
		//#endif
		
		//SwitchPlayerTeam(client, jointeam);
...
CheckPlayerInBombsite(client)
{
	if (GetEntProp(client, Prop_Send, "m_bInBombZone") > 0)
	{
		ReachedSite[client] = true;
	}
	else
	{
		ReachedSite[client] = false;
	}
}
...
public OnUseBotControlChanged(Handle:cvar, const String:oldVal[], const String:newVal[]) {
	...
	if (UseBotControl)
	{ // Maybe don't need 0.1 timer, maybe .05 or 1.0 would work?
		BC_Timer = CreateTimer(0.1, Timer_BRushMainLoop, _, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
	}
}
...
public OnStripColorsChanged(Handle:cvar, const String:oldVal[], const String:newVal[]) {
	StripColorsForCSGO = GetConVarBool(cvar);
}
